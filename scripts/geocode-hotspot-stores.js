...
async function main() {
    ...
    console.log('📦 Fetching Hotspot stores from database...\n');
    
    // Fetch only Hotspot stores
    const hotspotStores = await prisma.store.findMany({
        where: {
    ...
    console.log(`Found ${hotspotStores.length} Hotspot stores in total.\n`);
    
    let updated = 0;
    let failed = 0;

    for (let i = 0; i < hotspotStores.length; i++) {
        const store = hotspotStores[i];
        const cleanedName = cleanStoreName(store.storeName);
        
        // Custom prefix: "HOTSPOT MOBILE STORE"
        // Then store name
        // Then city
        // Then "India"
        const cityPart = store.city ? `, ${store.city}` : '';
        const query = `HOTSPOT MOBILE STORE ${cleanedName}${cityPart}, India`;
        
        process.stdout.write(`[${i + 1}/${hotspotStores.length}] Searching: "${query}" ... `);
    ...
    console.log(`\n✅ Finished Processing Hotspot Stores!`);
