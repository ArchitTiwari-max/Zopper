...
async function run() {
  try {
    ...
    console.log(`📊 Found ${mergePairs.length} store pairs marked for "Merge" in the spreadsheet.`);
    ...
    for (const { originalId, duplicateId } of mergePairs) {
      ...
      if (!O) {
        console.error(`❌ Original store with ID ${originalId} does not exist. Skipping merge for duplicate ID ${duplicateId}.`);
        continue;
      }
      if (!D) {
        console.warn(`⚠️ Duplicate store with ID ${duplicateId} does not exist in database (already deleted or merged). Skipping.`);
        continue;
      }
