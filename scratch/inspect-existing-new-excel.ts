...
function inspect() {
  ...
  if (sheetNames.length > 0) {
    ...
    if (data.length > 0) {
      ...
      const newStores = data.filter((row: any) => String(row['Store ID'] || '').toLowerCase().includes('new'));
      console.log("Number of stores with 'new' in Store ID:", newStores.length);
      if (newStores.length > 0) {
        console.log("Sample new stores:", JSON.stringify(newStores.slice(0, 3), null, 2));
      }

      // Check some stores that have numeric/existing Store ID
      const existingStores = data.filter((row: any) => {
      ...
      if (existingStores.length > 0) {
        console.log("Sample existing stores:", JSON.stringify(existingStores.slice(0, 3), null, 2));
      }
    } else {
