...
function styleHeaderCell(cell: ExcelJS.Cell, bgColor: string, fontColor = 'FFFFFFFF') {
...
export async function GET(request: Request) {
  try {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      dates.push(`${String(d).padStart(2, '0')}-${String(month).padStart(2, '0')}-${year}`);
    }
    ...
    if (categories.length > 0) {
      ...
      categories.forEach(c => wsCat.addRow([c.categoryName]));
    }
    ...
    return new NextResponse(buffer as Buffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="daily-sales-template-${brandName}-${monthStr}.xlsx"`
      }
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: error instanceof Error ? error.message : 'Failed to generate template' }),
