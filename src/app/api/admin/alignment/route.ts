...
const computeStoreData = (
  store: any,
  opts: { includeDrilldown: boolean; summaryOnly: boolean },
) => {
  const alignment = store.alignment;
  const storeType = store.storeName.toUpperCase().includes("CROMA")
    ? "Croma"
    : store.storeName.toUpperCase().includes("RELIANCE")
      ? "Reliance"
      : "Vijay Sales";
  ...
  if (alignment) {
    const storeLevel = Array.isArray(alignment.storeLevel)
      ? (alignment.storeLevel as any[])
      : [];
    const stakeholderLevel = Array.isArray(alignment.stakeholderLevel)
      ? (alignment.stakeholderLevel as any[])
      : [];
  ...
  const storeState = store.state || coords.state;
  ...
  if (opts.includeDrilldown && alignment) {
    result.storeLevel = alignment.storeLevel ?? [];
    result.stakeholderLevel = alignment.stakeholderLevel ?? [];
  }
 ...
 *
 * Query params:
 *   summaryOnly=true  → skip executiveStores join; fast map coloring
 *   state=StateName   → NDJSON stream; DB-level filter; full drilldown data
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // ── SINGLE STORE DETAIL — fetch full data for one store on DETAILS click ──
    ...
    if (storeId) {
      ...
      const data = computeStoreData(store, { includeDrilldown: true, summaryOnly: false });
      return NextResponse.json({ success: true, data });
    }

    // ── STATE DRILLDOWN — summary only (no storeLevel/stakeholderLevel) ─────────
    // Fast: only fetches stores for this state, returns lightweight list for the panel
    if (filterState) {
      const stateStores = await prisma.store.findMany({
        where: {
          state: {
            equals: filterState,
            mode: "insensitive",
        ...
        include: {
          alignment: true,
          // executiveStores not needed for summary list
        },
      });

      const data = stateStores
        .filter((store) => isBrandStore(store.storeName))
        .map((store) =>
          computeStoreData(store, { includeDrilldown: false, summaryOnly: false }),
        );

      return NextResponse.json({ success: true, data });
    }

    // ── SUMMARY / FULL — returns all stores as JSON ───────────────────────────
