      ...
      try {
        ...
        controller.enqueue(
          encoder.encode(
            `data: ${JSON.stringify({
              ...
              message:
                "✅ Cache initialized - 10x performance boost activated!",
            })}\n\n`,
          ),
        );
        ...
        for (let c = range.s.c; c <= range.e.c; ++c) {
          ...
          if (typeof baseHeader === "string") {
            const cleanHeader = baseHeader.trim();
            const slashMatch = cleanHeader.match(
              /^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/,
            );
            const hyphenMatch = cleanHeader.match(
              /^([0-9]{1,2})-([0-9]{1,2})-([0-9]{4})$/,
            );
            if (slashMatch) {
              const [_, dd, mm, yyyy] = slashMatch;
              baseHeader = `${dd.padStart(2, "0")}-${mm.padStart(2, "0")}-${yyyy}`;
            } else if (hyphenMatch) {
              const [_, dd, mm, yyyy] = hyphenMatch;
              baseHeader = `${dd.padStart(2, "0")}-${mm.padStart(2, "0")}-${yyyy}`;
            }
          }
          ...
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({
                type: "progress",
                message:
                  "âš ï¸ No data rows found in Excel file. Please check your file format.",
              })}\\n\\n`,
            ),
          );
