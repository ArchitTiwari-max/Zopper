...
export async function GET(request: NextRequest) {
  try {
    ...
    const executive = await prisma.executive.findUnique({
      ...
      select: { id: true, subordinates: { select: { id: true, name: true } } }
    });

    if (!executive) return NextResponse.json({ error: 'Executive not found' }, { status: 404 });

    const subordinateIds = executive.subordinates.map(s => s.id);
    if (subordinateIds.length === 0) {
      return NextResponse.json({ error: 'No subordinates found' }, { status: 404 });
    }
    ...
    if (executiveName !== 'All') {
      const matched = executive.subordinates.find(s => s.name === executiveName || s.name.split(' ')[0] === executiveName);
      if (matched) filteredSubIds = [matched.id];
    }

    const [visits, digitalVisits] = await Promise.all([
      visitType === 'Digital' ? Promise.resolve([]) :
        prisma.visit.findMany({
          where: { executiveId: { in: filteredSubIds }, visitDate: visitDateFilter },
          select: {
            id: true,
            status: true,
            personMet: true,
            ...
            store: { select: { id: true, storeName: true, storeBrands: { select: { brandId: true } } } },
            executive: { select: { name: true } },
            issues: { select: { id: true, details: true, status: true } }
          },
          orderBy: { visitDate: 'desc' }
        }),
      visitType === 'Physical' ? Promise.resolve([]) :
        prisma.digitalVisit.findMany({
          where: { executiveId: { in: filteredSubIds }, connectDate: visitDateFilter },
          select: {
            id: true,
            status: true,
            personMet: true,
    ...
    const allBrandIds = [...new Set([
      ...visits.flatMap(v => v.store?.storeBrands.map(sb => sb.brandId) || []),
      ...digitalVisits.flatMap(v => v.store?.storeBrands.map(sb => sb.brandId) || [])
    ])];
    const brands = allBrandIds.length > 0
      ? await prisma.brand.findMany({ where: { id: { in: allBrandIds } }, select: { id: true, brandName: true } })
      : [];
    const brandMap = new Map(brands.map(b => [b.id, b.brandName]));
    const getBrandString = (sb: { brandId: string }[]) =>
      sb.length > 0 ? sb.map(s => brandMap.get(s.brandId) || 'Unknown').join(', ') : 'N/A';
    ...
    const formatPeopleMet = (personMet: any): string => {
      ...
      return personMet.map((p: any) => `${p.name}${p.designation ? ` (${p.designation})` : ''}${p.phoneNumber ? ` - ${p.phoneNumber}` : ''}`).join('; ');
    };
    ...
    const rows = [
      ...visits.map(v => ({
        ...
        'Status': v.status,
        'POSM Available': v.POSMchecked === null ? 'N/A' : v.POSMchecked ? 'Yes' : 'No',
        'People Met': formatPeopleMet(v.personMet),
        'Remarks': v.remarks || '',
        'Issues Count': v.issues?.length || 0,
        'Issue Details': v.issues?.map((i: any) => `${i.details} [${i.status}]`).join('; ') || '',
      ...
      const da = new Date(a['Visit Date'].split('/').reverse().join('-')).getTime();
      const db = new Date(b['Visit Date'].split('/').reverse().join('-')).getTime();
      return db - da;
    });
