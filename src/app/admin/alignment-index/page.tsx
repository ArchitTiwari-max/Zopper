...
const StoreDetailView = ({
    ...
    // Normalize — storeLevel/stakeholderLevel may be absent on summary-only stores
    const storeLevel: any[] = Array.isArray(store.storeLevel) ? store.storeLevel : [];
    const stakeholderLevel: any[] = Array.isArray(store.stakeholderLevel) ? store.stakeholderLevel : [];

    // Alignment logic for the new DB-backed JSON structure
    const allStoreAligned =
        storeLevel.length > 0 &&
        storeLevel.every(
            (entry: any) => entry.personnel && entry.personnel.length > 0,
        );
    const allStakeholderAligned =
        stakeholderLevel.length > 0 &&
        stakeholderLevel.every(
            (entry: any) => entry.personnel && entry.personnel.length > 0,
        );
                ...
                <div className="sd-title-block">
                    ...
                    <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                            ...
                            <span
                                className={`sd-type-badge ${store.storeType === "Croma" ? "croma" : store.storeType === "Vijay Sales" ? "vijay-sales" : "reliance"}`}
                            >
                                {store.storeType}
                            </span>
                        </div>
                        <span className="sd-store-code">
                            {store.code} &nbsp;·&nbsp; {store.city}
                        </span>
                    </div>
                    <div className={`sd-score-pill ${store.alignment}`}>
                        {store.score}%
                    </div>
                </div>
            </div>
                ...
                {activeTab === "store" && (
                    <div className="sd-section">
                        <div className="sd-section-label">
                            ...
                            <span
                                className={`sd-overall-badge ${allStoreAligned ? "aligned" : "misaligned"}`}
                            >
                        ...
                        <div className="sd-role-table">
                            ...
                            {storeLevel.map((entry: any, idx: number) => {
                                const isAligned = entry.personnel && entry.personnel.length > 0;
                                return (
                                    <div
                                        key={idx}
                                        className={`sd-table-row-group ${isAligned ? "aligned" : "misaligned"}`}
                                    >
                                        <div className="sd-table-row header-row">
                                            <span className="sd-role-tag">{entry.role}</span>
                                            <span className="sd-person-count">
                                                {isAligned
                                                    ? `${entry.personnel.length} Assigned`
                                                    : "Vacant"}
                                            </span>
                                            <span
                                                className={`sd-status-chip ${isAligned ? "aligned" : "misaligned"}`}
                                            >
                                                {isAligned ? (
                                                    <>
                ...
                {activeTab === "stakeholder" && (
                    <div className="sd-section">
                        <div className="sd-section-label">
                            ...
                            <span
                                className={`sd-overall-badge ${allStakeholderAligned ? "aligned" : "misaligned"}`}
                            >
                        ...
                        <div className="sd-role-table">
                            ...
                            {stakeholderLevel.map((entry: any, idx: number) => {
                                const isAligned = entry.personnel && entry.personnel.length > 0;
                                return (
                                    <div
                                        key={idx}
...
const AlignmentIndexPage = () => {
    ...
    // Tracks the in-flight drilldown fetch so we can abort it on rapid state switches
    const drilldownAbortRef = useRef<AbortController | null>(null);

    // ── Stage 1: Fast map load — skip executiveStores join (~2s) ─────────────
    useEffect(() => {
        const fetchSummary = async () => {
            try {
                const response = await fetch("/api/admin/alignment?summaryOnly=true");
    ...
    // ── Stage 2: Full data in background — upgrades executives count ──────────
    useEffect(() => {
        const fetchFull = async () => {
            try {
                ...
                console.error("Error fetching full alignment data:", error);
            }
        };
        fetchFull();
    }, []);
    ...
    const stateData = React.useMemo(() => {
        ...
        stores.forEach((store) => {
            const stateKey = store.state
                ...
                ?.trim();
            const stateId = stateKey ? nameToId[stateKey] : undefined;
    ...
    const handleStateClick = async (id: string) => {
        // Cancel any in-flight drilldown request immediately
        ...
        if (stateName) {
            try {
                const response = await fetch(
                    `/api/admin/alignment?state=${encodeURIComponent(stateName)}`,
    ...
    const handleMarkerClick = async (store: any) => {
        ...
        // Fetch full detail for this single store via ?storeId=
        try {
            const res = await fetch(`/api/admin/alignment?storeId=${encodeURIComponent(store.id)}`);
            const json = await res.json();
            if (json.success) {
                setSelectedStore(json.data);
            }
        } catch (error) {
    ...
    return (
        <div
            ...
            style={{
                paddingRight:
                    drilldownState || detailStore ? "min(420px, 30%)" : undefined,
        ...
        >
            ...
            <div className="hud-header">
                    ...
                    <div className="header-content">
                        ...
                        <div>
                            <h1 className="hud-title">
                                EXECUTIVE ALIGNMENT <span className="highlight">MATRIX</span>
                            </h1>
                ...
                <div className="hud-sub-header">
                    <div className="view-toggle">
                        <button
                            className={`cy-toggle-btn ${viewMode === "map" ? "active" : ""}`}
                            onClick={() => setViewMode("map")}
                        >
                            MAP VIEW
                        </button>
                        <button
                            className={`cy-toggle-btn ${viewMode === "all" ? "active" : ""}`}
                ...
                <>
                    ...
                    <div className="hud-map-frame">
                        {loading && (
                            <div className="map-loading-overlay">
                                ...
                                <p>GRIDDING GEO-SPATIAL NETWORK...</p>
                            </div>
                        )}
                                ...
                                <div className="hologram-popup">
                                    <div className="hologram-header">
                                        ...
                                        <div className="hologram-title">
                                            ...
                                            <span className="hologram-id">
                                                ID: {selectedStore.id.toString().padStart(4, "0")}
                                            </span>
                                        </div>
                                        <button className="hologram-close" onClick={handleReset}>
                                            &times;
                                        </button>
                                    </div>
                    ...
                    {hoveredState && stateData[hoveredState.id] && viewMode === "map" && (
                        <div
                            ...
                            style={{
                                left: `${hoveredState.x + 15}px`,
                                top: `${hoveredState.y + 15}px`,
                            }}
                        >
                                ...
                                <div
                                    ...
                                    style={{
                                        backgroundColor: getStateColor(
                                            stateData[hoveredState.id].score,
                                            true,
                                        ),
                                    }}
                                >
                                    {stateData[hoveredState.id].score !== undefined
                                        ? `${stateData[hoveredState.id].score}%`
                                        : "N/A"}
                                </div>
                            </div>
                            <div className="tooltip-body">
                                ...
                                >
                                    ...
                                    <strong style={{ color: "#fff" }}>
                                        {stateData[hoveredState.id].stores}
                                    </strong>
                                </div>
                                <div
                                    className="tooltip-row"
                                ...
                                >
                                    ...
                                    <span className="text-alert" style={{ fontWeight: "bold" }}>
                                        {stateData[hoveredState.id].critical}
                                    </span>
                                </div>
                                <div
                                    className="tooltip-row"
                                ...
                                >
                                    <span className="text-warning" style={{ fontWeight: "bold" }}>
                                        {stateData[hoveredState.id].missingRoles} Gap Stores
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            {viewMode === "all" && (
                ...
                >
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
                        {(() => {
                            // ── Loading skeleton — shown while summaryOnly fetch is in-flight ──
                            if (loading) {
                                return (
                                    <div
                                        style={{
                                        ...
                                        {Array.from({ length: 8 }).map((_, i) => (
                                            ...
                                            >
                                                ...
                                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                    <div style={{ width: "55%", height: "18px", borderRadius: "4px", background: "rgba(255,255,255,0.07)" }} />
                                                    <div style={{ width: "48px", height: "24px", borderRadius: "4px", background: "rgba(255,255,255,0.07)" }} />
                                                </div>
                                                {/* Stats row placeholder */}
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <div style={{ width: "40%", height: "12px", borderRadius: "4px", background: "rgba(255,255,255,0.05)" }} />
                                                    <div style={{ width: "25%", height: "12px", borderRadius: "4px", background: "rgba(255,255,255,0.05)" }} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            if (searchQuery) {
                                ...
                                if (filteredStores.length === 0) {
                                    return (
                                        ...
                                        >
                                            NO STORES FOUND MATCHING SCAN CRITERIA.
                                        </div>
                                    );
                                }
                                    ...
                                    >
                                        {filteredStores.map((store) => (
                                            ...
                                            >
                                                ...
                                                >
                                                    <div
                                                        className={`mini-score ${store.alignment}`}
                                                        style={{
                                                            fontWeight: "bold",
                                                            padding: "4px 8px",
                                                            borderRadius: "4px",
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={async () => {
                                                            setDetailStore(null);
                                                            console.log("[DETAILS] Fetching store:", store.id);
                                                            try {
                                                                const res = await fetch(
                                                                    `/api/admin/alignment?storeId=${encodeURIComponent(store.id)}`,
                                                                );
                                                                const json = await res.json();
                                                                console.log("[DETAILS] Response:", json);
                                                                setDetailStore(json.success ? json.data : null);
                                                            } catch (err) {
                                    ...
                                    {activeStates.map(([id, data]) => (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {data.missingRoles !== undefined &&
                                                    data.missingRoles > 0 && (
                                                        <span className="text-warning">
                                                            {data.missingRoles} Gap
                                                            {data.missingRoles > 1 ? "s" : ""}
                                                        </span>
                                                    )}
                                            </div>
                                        </div>
            ...
            <div
                className={`drilldown-panel ${drilldownState || detailStore ? "open" : ""}`}
            >
                                ...
                                <div className="panel-header">
                                    <div>
                                        <h2 className="panel-title">
                                            {stateData[drilldownState]?.name || "State Detail"}
                                        </h2>
                                        <p className="panel-subtitle">
                                            Regional Alignment Intelligence
                                        </p>
                                    </div>
                                    <button
                                                                ...
                                                                <path
                                                                    ...
                                                                    style={{
                                                                        stroke: getStateColor(
                                                                            stateData[drilldownState]?.score,
                                                                            true,
                                                                        ),
                                                                    }}
                                                                    d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
