...
const StoreVisitsPage: React.FC = () => {
  ...
  const [selectedVisit, setSelectedVisit] = useState<VisitCardData | null>(
    null,
  );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [digRes, phyRes, issuesRes] = await Promise.all([
        fetch(`/api/admin/digital-report/data?${qs.toString()}`, {
          credentials: "include",
        }),
        fetch(`/api/admin/visit-report/data?${qs.toString()}`, {
          credentials: "include",
        }),
        fetch(
          `/api/admin/issues/data?${new URLSearchParams({ dateFilter: "All Time", storeId, status: "Pending" }).toString()}`,
          { credentials: "include" },
        ),
      ]);

      if (!digRes.ok) {
        const err = await digRes
          .json()
          .catch(() => ({ error: `Digital HTTP ${digRes.status}` }));
        throw new Error(err.error || `Digital HTTP ${digRes.status}`);
      }
      if (!phyRes.ok) {
        const err = await phyRes
          .json()
          .catch(() => ({ error: `Physical HTTP ${phyRes.status}` }));
        throw new Error(err.error || `Physical HTTP ${phyRes.status}`);
      }
      if (!issuesRes.ok) {
        const err = await issuesRes
          .json()
          .catch(() => ({ error: `Issues HTTP ${issuesRes.status}` }));
      ...
      setOpenIssues(
        (issuesData.issues || []).filter((i: any) => i.status !== "Resolved"),
      );
    } catch (e) {
  ...
  const Section: React.FC<{
    ...
    >
      <h3
        style={{ margin: 0, marginBottom: 12, fontSize: 18, fontWeight: 700 }}
      >
        {title}
      </h3>
      {isLoading ? (
          ...
          {visits.map((v) => (
            ...
            >
              ...
              >
                ...
                <span
                  style={{
                    background: getStatusColor(v.visitStatus),
                    color: "#fff",
                    padding: "2px 8px",
                    borderRadius: 999,
                    fontSize: 12,
              ...
              {v.peopleMet && v.peopleMet.length > 0 && (
                ...
                >
                  ...
                  {v.peopleMet
                    .map(
                      (p, i) =>
                        `${p.name} (${p.designation})${p.phoneNumber ? ` - ${p.phoneNumber}` : ""}`,
                    )
                    .join(", ")}
                </div>
              )}
                ...
                >
                  ...
                  >
                    ...
                    {v.issueStatus && (
                      <span
                        style={{
                          background: getIssueColor(v.issueStatus),
                          color: "#fff",
                          padding: "2px 8px",
                          borderRadius: 999,
                          fontSize: 11,
              ...
              >
                <button
                  onClick={() => openDetails(v, isDigital)}
                  style={{
                    padding: "6px 10px",
                    borderRadius: 6,
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Section
        title="Last 5 Physical Visits"
        visits={physicalVisits}
        isDigital={false}
      />
      <Section
        title="Last 5 Digital Visits"
            ...
            {openIssues.map((iss) => (
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      background: getIssueColor(iss.status),
                      color: "#fff",
                      padding: "2px 8px",
                      borderRadius: 999,
                      fontSize: 12,
                ...
                <div
                  style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}
                >
                  Reported: {iss.dateReported} • By: {iss.reportedBy}
                </div>
                <div
                  style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}
                >
                  Brand: {iss.brandAssociated}
                </div>
                {iss.description && (
